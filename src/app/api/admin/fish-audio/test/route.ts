...
export async function POST(req: NextRequest) {
    try {
        ...
        const { data, error } = await supabase
            .from("bot_settings")
            .select("key,value")
            .in("key", ["elevenlabs_api_key", "elevenlabs_voice_id", "elevenlabs_model"]);
        ...
        const settings = normalizeElevenLabsSettings({
            apiKey,
            enabled: true,
            voiceId: String(body.fishAudioVoiceId || map.elevenlabs_voice_id || process.env.ELEVENLABS_VOICE_ID || DEFAULT_ELEVENLABS_SETTINGS.voiceId),
