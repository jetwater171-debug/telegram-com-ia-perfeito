...
const emptySettings: AiSettings = {
    ...
    aiModelOrder: "",
    aiStrategyModelOrder: "",
    aiDraftModelOrder: "",
    aiReviewModelOrder: "",
    aiEvaluatorModelOrder: "",
};
        ...
        <div className="min-h-screen bg-[#080b10] text-slate-100">
            <header className="sticky top-0 z-30 border-b border-white/10 bg-[#080b10]/95 backdrop-blur">
                <div className="mx-auto flex w-full max-w-6xl flex-col gap-4 px-4 py-4 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                        ...
                        <p className="text-sm text-slate-400">Chaves, ordem de fallback e saúde dos modelos</p>
                    </div>
            ...
            <main className="mx-auto grid w-full max-w-6xl gap-5 px-4 py-6 lg:grid-cols-[1.2fr_0.8fr]">
                <section className="space-y-5">
                    <div className="rounded-lg border border-white/10 bg-white/[0.04] p-5">
                        <div className="flex items-start justify-between gap-3">
                            <div>
                                ...
                                <p className="text-sm text-slate-400">Preencha só quando quiser trocar. Chave salva não aparece inteira.</p>
                            </div>
                        ...
                        <div className="mt-4 grid gap-4 md:grid-cols-2">
                            <Field label={`OpenRouter ${settings.openrouterApiKeyMasked ? `(${settings.openrouterApiKeyMasked})` : "(não salvo)"}`}>
                    ...
                    <div className="rounded-lg border border-white/10 bg-white/[0.04] p-5">
                        <h2 className="text-lg font-semibold">Ordem das IAs</h2>
                        <p className="mt-1 text-sm text-slate-400">Separe por vírgula. A primeira tenta primeiro; se falhar, cai para a próxima.</p>
                        ...
                        <div className="mt-4 grid gap-4">
                            ...
                            <TextArea label="Ordem global extra" value={settings.aiModelOrder} onChange={(v) => update("aiModelOrder", v)} placeholder="opcional: openrouter:modelo,gemini:gemini-2.5-flash" />
                        </div>
                ...
                <aside className="space-y-5">
                    <div className="rounded-lg border border-white/10 bg-white/[0.04] p-5">
                        <h2 className="text-lg font-semibold">Saúde das IAs</h2>
                        ...
                        <div className="mt-4 rounded-lg border border-red-400/20 bg-red-400/10 p-3 text-sm">
                            ...
                            <p className="mt-1 font-semibold text-red-100">{worstModel ? `${roleLabels[worstModel.role] || worstModel.role} - ${worstModel.model}` : "Sem dados"}</p>
                            <p className="text-xs text-red-100/70">{worstModel ? `${worstModel.error || 0} erros` : "Ainda não teve chamada registrada"}</p>
                        </div>
                    </div>
                    ...
                    <div className="rounded-lg border border-white/10 bg-white/[0.04] p-5">
                        ...
                        <div className="mt-3 max-h-72 overflow-auto space-y-2">
                            {stats.length === 0 && <p className="text-sm text-slate-400">Sem estatística ainda.</p>}
                            {stats.map((item) => (
                        ...
                        <div className="mt-3 max-h-[420px] overflow-auto space-y-2">
                            ...
                            {recentEvents.map((event, index) => (
                                <div key={`${event.at}-${index}`} className="rounded-lg border border-white/10 bg-black/25 p-3 text-xs">
                                    ...
                                    <p className="mt-1 text-slate-200">{roleLabels[event.role] || event.role} - {event.model}</p>
...
function TextArea({ label, value, onChange, placeholder }: { label: string; value: string; onChange: (value: string) => void; placeholder?: string }) {
    return (
        <label className="grid gap-2 text-sm">
            <span className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">{label}</span>
            <textarea value={value} onChange={(e) => onChange(e.target.value)} rows={2} placeholder={placeholder} className="min-h-16 rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-sm text-slate-100 outline-none focus:border-cyan-300/60" />
        </label>
    );
}
