...
const providerDefaults = (provider: string, model: string): GatewayRatePolicy => {
    ...
    if (normalizedProvider === 'roteia' || normalizedProvider === 'llm7') {
        return unknownQuota({ maxConcurrency: 4, timeoutMs: 30_000, maxQueueMs: 2_500 });
    }
...
export const resolveGatewayLatencyBudget = ({
    ...
    const totalMs = auxiliary ? 6_000 : operationalRepair ? 16_000 : 38_000;
    const providerAttemptMs = provider === 'bai'
        ? 10_000
        : provider === 'nvidia'
            ? 16_000
            : (provider === 'roteia' || provider === 'llm7')
                ? 30_000
            : 18_000;
