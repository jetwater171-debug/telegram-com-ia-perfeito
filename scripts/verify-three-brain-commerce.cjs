...
assert.match(prompts, /Antes de finalizar, confira silenciosamente/);
assert.doesNotMatch(gateway, /strategyCallPromise/);
assert.match(prompts, /LARI — MASTER BRAIN DE CONVERSA/);
assert.match(prompts, /messages deve conter de 1 a 4 balões ordenados/);
assert.match(gateway, /thinking = \{ type: 'disabled' \}/);
assert.doesNotMatch(gateway, /const isRetryable = gateway\.provider === 'bai'/);
assert.match(gatewayRouter, /timeoutMs: readLimit\(env, provider, 'TIMEOUT_MS', defaults\.timeoutMs\)/);
...
assert.match(prompts, /action solicita no máximo uma operação; o backend ainda valida e executa/);
assert.match(route, /modelCanPriceCustom/);
assert.match(route, /buildModelPricedCustomOffer/);
assert.match(route, /offerPlan = modelPricedOffer/);
