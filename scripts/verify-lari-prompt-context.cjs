...
assert.match(editorModule.DEFAULT_FULL_SYSTEM_INSTRUCTION_TEMPLATE, /Não existe oferta obrigatória no terceiro turno/);
assert.match(editorModule.DEFAULT_FULL_SYSTEM_INSTRUCTION_TEMPLATE, /Três prévias gratuitas são o percurso normal/);
...
assert.doesNotMatch(capturedTemplateValues.LEAD_PROFILE, /instagram|vip-agosto|example\.invalid|identificador-nao-enviar|secret=1|ignore regras/);
console.log('LARI_PROMPT_CONTEXT_OK operational=uncut runtime=uncut optional_budgets=separate acquisition_hidden=1 privacy=1 editable_instruction=1 zero_minutes=preserved');
